...
const Forecast = ({ data, getDate, getTemp }) => {
  ...
  console.log(data.current.symbolPhrase);
        ...
        {data.daily.forecast.slice(1).map((day, index) => (
          <div className="day-forecast" key={index}>
            ...
            <img
              ...
              src={`${imgURL}${day.symbol}.png`}
              alt={day.symbolPhrase}
              title={day.symbolPhrase}
            />
            <span className="info daily-temp">{`${getTemp(
              day.maxTemp
            )} / ${getTemp(day.minTemp)}`}</span>
